...
-(id)initWithFrame:(CGRect)frame withPhotoUrl:(NSString *)photoUrl{
    ...
    if (self) {
        ...
        if (!isCached) {//没有缓存
            ...
            [self.imageView sd_setImageWithURL:[NSURL URLWithString:photoUrl] placeholderImage:DefaultContentImage options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize){
