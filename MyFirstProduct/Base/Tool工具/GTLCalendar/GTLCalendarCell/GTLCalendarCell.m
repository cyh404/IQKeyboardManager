...
- (void)setIsFromDate:(BOOL)isFromDate {
    ...
    if (isFromDate) {
        ...
        CAShapeLayer *shapeLayer = [CAShapeLayer layer];
        shapeLayer.path = path.CGPath;
        shapeLayer.fillColor = [UIColor colorWithRGBHexString:@"F05D2D"].CGColor;//[UIColor colorWithRed:245.0/255.0 green:162.0/255.0 blue:27.0/255.0 alpha:1].CGColor;
        if (self.shapeLayer) {
            [self.contentView.layer insertSublayer:shapeLayer above:self.shapeLayer];
        }
        else {
...
- (void)setIsToDate:(BOOL)isToDate {
    ...
    if (isToDate) {
        ...
        CAShapeLayer *shapeLayer = [CAShapeLayer layer];
        shapeLayer.path = path.CGPath;
        shapeLayer.fillColor = [UIColor colorWithRGBHexString:@"F05D2D"].CGColor;//[UIColor colorWithRed:233.0/255.0 green:97.0/255.0 blue:75.0/255.0 alpha:1].CGColor;
        if (self.shapeLayer) {
            [self.contentView.layer insertSublayer:shapeLayer above:self.shapeLayer];
        }
        else {
