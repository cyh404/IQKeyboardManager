...
-(void)setImagesDeselected:(NSString *)deselectedImage
    ...
    CGFloat startWidth = 8;
    CGFloat pading = 8;
    width = (self.frame.size.width-4*pading-2*startWidth)/5;
    height = width;
    CGFloat startHeight = (self.frame.size.height-height)/2;
	starRating = 0;
	lastRating = 0;
	...
	[s1 setFrame:CGRectMake(startWidth,         startHeight, width, height)];
	[s2 setFrame:CGRectMake(startWidth+(width+pading),     startHeight, width, height)];
	[s3 setFrame:CGRectMake(startWidth+2 * (width+pading), startHeight, width, height)];
	[s4 setFrame:CGRectMake(startWidth+3 * (width+pading), startHeight, width, height)];
	[s5 setFrame:CGRectMake(startWidth+4 * (width+pading), startHeight, width, height)];
...
-(void) touchesMoved: (NSSet *)touches withEvent: (UIEvent *)event
{
    ...
    CGFloat startWidth = 8;
    CGFloat pading = 8;
	int newRating = (int) ((pt.x-startWidth) / (width+pading)) + 1;
...
+ (instancetype)standardRatingViewWithDelegat:(id<RatingViewDelegate>)delegate{
    RatingView *ratingView = [[RatingView alloc] initWithFrame:CGRectMake(0, 0, 100, 30)];
    [ratingView setImagesDeselected:@"img_star" partlySelected:@"img_star" fullSelected:@"img_star_pre" andDelegate:delegate];
    return ratingView;
}
