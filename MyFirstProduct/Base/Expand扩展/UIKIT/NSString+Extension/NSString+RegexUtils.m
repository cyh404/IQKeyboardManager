...
#import "NSString+RegexUtils.h"

@implementation NSString (RegexUtils)
...
-(BOOL)isMACAddress{
    NSString *emailRegex =@"([A-Fa-f0-9]{2}:){5}[A-Fa-f0-9]{2}";
...
- (BOOL)isValidateIDCardNumber {
    ...
    value = [value stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];//去除空格
...
-(BOOL)judgePassWord{
    ...
    if ([self length] >= 6){
        ...
        NSString *regex = @"^(?=.{6,12})(?=.*[a-z])(?=.*[0-9])[0-9a-z]*$";
